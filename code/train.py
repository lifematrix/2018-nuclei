...
def padding_array(ary, offset, default_val):
    ...
    ary_padded = np.pad(ary, ((0, 0), (p1, p2),(p1, p2)), "constant", constant_values=default_val)
...
def main():
    ...
    trainer = unet.Trainer(net, batch_size=batch_size, optimizer="momentum",
                           opt_kwargs=dict(momentum=0.8, learning_rate=0.01))
    path = trainer.train(data_provider, "log/20180414",
                         training_iters=training_iters, epochs=30, display_step=2)
